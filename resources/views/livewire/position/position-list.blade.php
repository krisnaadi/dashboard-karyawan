            ...
            <table class="table">
                ...
                <tbody>
                    @forelse ($this->positions as $position)
                        <tr class="hover">
                            <th>
                                {{ $loop->iteration + ($this->positions->currentPage() - 1) * $this->positions->perPage() }}
                            </th>
                            <td>{{ $position->name }}</td>
                            <td width="200px">
                                <div class="flex gap-2">
                                    <button class="btn btn-square btn-ghost btn-sm" wire:click="edit({{ $position->id }}); modalOpen=true">
                                        <x-heroicon-o-pencil class="w-5 h-5" />
                                    </button>
                                    <button class="btn btn-square btn-ghost btn-sm" wire:click="delete({{ $position->id }})"
